...
`bin/crt-stt-confidence.py` — the decision function only (normalize_key,
call_probability, should_call_claude, record_confirmed/record_claude_call,
persisted to `~/.crt/stt-confidence.json`) — see its own header and
docstrings.
...
- Every playbook handler returns its spoken/summary text (previously
  ...
  against.
- `confidence_route()`, `_confirm_in_background()`, and `_answers_match()`
  carry the actual wiring (the flag, the background thread, the loose
  match) — see their own docstrings.
