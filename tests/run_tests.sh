...
echo "== stt-feed.sh CRT_STT_GATE opt-in gate + voice-control keystroke mapping =="
...
echo "== crt-stt-stream.py LocalAgreement commit logic + read_exact =="
python3 "$DIR/test_stt_stream_helpers.py" || fail=1
echo
...
echo "== crt-self-repair.sh commits survive a crashing claude -p (crt#48) =="
bash "$DIR/test_self_repair.sh" || fail=1
echo

echo "== crt-pull.sh fast-forward-only pull + targeted window restart (crt#325) =="
bash "$DIR/test_crt_pull.sh" || fail=1
echo

echo "== crt-potato-status.sh status snapshot (crt#325) =="
bash "$DIR/test_potato_status.sh" || fail=1
echo

echo "== crt_console_windows.py: window map derived from crt-console.sh itself (crt#325) =="
python3 -m unittest discover -s "$DIR" -p "test_crt_console_windows.py" -v 2>&1 | tail -10 || fail=1
echo
