#!/usr/bin/env python3
# The arm window is measured on the WRONG CLOCK (2026-07-29, nightly-batch).
#
# .claude/FOCUS.md's stability bar has carried this item unchecked since
# 2026-07-23 with a live-verify note from 07-28 that reads, verbatim: "every
# attempt that actually omitted 'potato' arrived 16s+ after its preceding wake
# ... past the 12s CRT_WAKE_ARM_SECS window ... Real open question, not just an
# untested checkbox: is 12s tight for natural pacing, or is the window's clock
# starting from the wrong reference point given lag?"
#
# It is the reference point, and the evidence is in this room's own log.
# Measured on potato's live ~/.crt/stt.log this run (1597 consecutive-utterance
# gaps): the median gap between one transcript landing and the next is 21s,
# with a hard mode at 20-22s -- because CRT_VAD_MAX cuts continuous speech at
# 20s, so a run-on wake-then-follow-up is emitted as two utterances ~20s apart
# in transcript time while the actual silence between them is under a second.
# Only 19% of gaps were under the 12s window at all. The window was never
# too short; emit() was comparing it against a number made mostly of the
# follow-up's own speaking duration plus two whisper round-trips (1-3s each
# healthy, worse when the potato<->mandark link is flaky -- the same
# link-reliability issue .claude/QUESTIONS.md already carries).
#
# So: the openness test now runs against when the follow-up STARTED being
# spoken, and the deadline is anchored to when the arming utterance FINISHED
# being spoken. Both instants are captured in the capture loop before
# transcribe() is ever called.
#
# NOT A LIVE CONFIRMATION. These are unit tests over real recorded numbers.
# What is proven here is that the window now measures silence instead of
# round-trips; whether 12s of silence is the right feel is still Zach's ear.
...
class FakeClock:
    """Stands in for the `time` module inside crt-wake-arm.py only. Every test
    below leaves it parked at the LANDED instant -- i.e. at what emit()'s own
    clock would say -- so that any code path still reading it instead of the
    passed-in audio timestamps fails loudly rather than coincidentally."""
...
class TestTwoClocks(unittest.TestCase):
    ...
    def test_a_long_followup_is_judged_on_when_it_started(self):
        ...
        # does not land until 126. Before this change the 126 was the number
        # compared, and this utterance -- spoken one second after the wake
        # word -- was gate-dropped in silence.
    ...
    def test_the_old_transcript_time_comparison_would_have_dropped_it(self):
        # A WITNESS, not a regression guard. Same room, same two utterances,
        # compared the way emit() used to compare them -- both ends taken at
        # the instant the transcript landed. It is refused, one second of real
        # silence after the wake word. That is the live symptom FOCUS.md
        # recorded on 2026-07-28, reproduced here so the change above cannot
        # be read as a no-op. Uses only pre-2026-07-29 arguments on purpose:
        # this test passes against the parent too, and should.
        state = self.wa.ArmState()
        state.arm("potato this is zach", "exact", None, "potato",
    ...
    def test_the_window_it_slides_to_is_anchored_on_the_END_of_the_speech(self):
        # Anchoring the NEXT window on the onset instead would hand back a
        # deadline of 117 -- six seconds before the person even stopped
        # talking, i.e. a window already closed on arrival.
        state = self.armed_state()
    ...
    def test_the_session_ceiling_still_holds_against_long_utterances(self):
        # Sliding on end-of-speech gives each follow-up MORE of the ceiling
        # than sliding on onset would. The ceiling itself must still stop an
        # unbounded ratchet -- CLAUDE.md's premise is a room full of chatter.
        state = self.wa.ArmState()
    ...
    def test_omitting_ended_at_is_exactly_the_old_behaviour(self):
        # Every pre-2026-07-29 caller and test passes only `now`. That has to
        # keep meaning what it meant, or this change is a silent rewrite of
        # decisions Zach already confirmed (see arm()'s docstring).
        state = self.armed_state()
        self.assertTrue(self.wa.consume_arm_with_followup(
...
class TestTimeoutDefersToAnOpenUtterance(unittest.TestCase):
    """check_arm_timeout() runs on the capture loop's own wall clock, which
    keeps ticking while someone is mid-sentence. With the deadline now
    anchored to audio time, it can pass while the follow-up is still being
    SPOKEN -- CRT_VAD_MAX allows 20s of speech against a 12s window -- so
    without a deferral the loop would disarm the window a few seconds before
    emit() got the chance to consume it. That would be the original bug back
    ...
    def test_an_utterance_that_began_after_the_deadline_earns_no_deferral(self):
        # It could never have been consumed, so holding the window open for it
        # would just be a longer window by accident.
        self.assertTrue(self.wa.check_arm_timeout(
...
class TestThroughTheLiveEmitPath(unittest.TestCase):
    """crt-stt-solo.py's real emit(), real gate, real wiring -- only the sinks
    and crt-wake-arm.py's clock are faked. The clock is deliberately parked at
    the instant the TRANSCRIPT landed, so a regression back to reading it
    reproduces the live symptom rather than raising."""
