...
exit 0
SH
chmod +x "$BLINDTMP/bin/docker"

blind_out="$(PATH="$BLINDTMP/bin:$PATH" ZAXON_COMPOSE_DIR="$BLINDTMP" bash "$A" --check 2>"$BLINDTMP/err")"
blind_rc=$?

[ "$blind_rc" -eq 6 ] \
  && ok "exits 6 BLIND when the registry read fails, not 0" \
  || bad "exit was $blind_rc, expected 6"

echo "$blind_out" | grep -q '^  BLIND' \
  && ok "reports BLIND for the unreadable image" \
  || bad "did not report BLIND: $blind_out"

echo "$blind_out" | grep -qE '^  same|up to date' \
