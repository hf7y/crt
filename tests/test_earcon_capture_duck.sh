...
# device -- can't fix the missing signal, but keeps a played tone from being
# misread as speech by the VAD). See crt-earcon.sh's own capture_mute()
# comments for the device-detection rationale.
set -uo pipefail
