...
- [ ] Book Game funnel re-verified end-to-end — **offline half still passing** (`tests/test_console_book_game_layout.sh`, re-run 2026-07-28, all PASS). Live half genuinely needs a human hand on the real barcode scanner — confirmed 2026-07-28 the `book` tmux window is live and correctly prompting ("0 book(s) registered — scan o[ne]"), but nothing remote can produce a real scan event. Ranked-backlog item 4, unchanged: needs Zach with a physical book.
  ...
  **2026-07-28 (realisateur, Zach-directed): earcon room-alert toggle wired, HDMI audibility unresolved — needs Zach's hands.** `crt-earcon.sh` already supported `--device tv` (`plughw:2,0`, HDMI) alongside `handset`, but `crt-secretary.py`'s own `play_earcon()` never passed `--device` at all, silently falling through to the ALSA system default regardless of what `crt-stt-solo.py`'s earcons were routed to. Fixed: both call sites now read the same `CRT_EARCON_DEVICE` env var (default `handset`, unchanged). Set `CRT_EARCON_DEVICE=tv` in potato's `~/.bash_profile` as the room-wide **testing default** (toggle: unset or set back to `handset` to revert) — durable across reboots, not just this session. **Audibility itself is unconfirmed and possibly a real gap, not just an untested one:** `crt-earcon-loopback-test.py tv` measured a real signal (2.9x baseline) but below the tool's 8.0x detection threshold; card 2 (`vc4-hdmi`) has no ALSA mixer/volume controls at all (normal for HDMI passthrough); `/sys/class/drm/card0-HDMI-A-1/status` reports `connected` with a live 256-byte EDID, so *something* is physically plugged into the HDMI port. But `POTATO.md` documents potato's actual display path as **composite/RF to the CRT, not HDMI** — so what's connected to HDMI, and whether it has a speaker in the room at all, is unknown from here. Zach reported not hearing the test tone. **Needs Zach to physically check what's on potato's HDMI port** before this checkbox can close; the software mechanism (toggle + both call sites wired) is done regardless of the answer.
