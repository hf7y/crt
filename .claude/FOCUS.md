...
Done when:
- [ ] sticky-conversation-window / arm-window state machine — **code done** (`consume_arm_with_followup()`/`check_arm_timeout()` implemented in `bin/crt-wake-arm.py`, wired into `crt-stt-solo.py`, tested in `tests/test_wake_arm.py`, commit `ede80b5`, 2026-07-23 — this checklist line was stale) — still needs live-confirm on potato (`CRT_WAKE_ARM_ENABLED` default OFF, `CRT_WAKE_ARM_SECS=12` untuned by ear)
- [ ] handset play-while-capture bug — **code done** (CTL-file mute/unmute duck around handset playback in `crt-tts.py`/`crt-earcon.sh`, plus a real bug fix: `CRT_CTL_FILE` was never exported in `crt-console.sh`'s boot path so the whole CTL-file mechanism was silently inert on potato — commit `fe46ac1`, 2026-07-24) — still needs a live re-run of `crt-earcon-loopback-test.py handset` on potato (unreachable as of 2026-07-24, see `.claude/QUESTIONS.md`) or Zach's own ear
- [ ] capture device resolved by NAME — **code done** (`resolve_capture_device_by_name()`/`_detect_capture_device()` in `crt-stt-solo.py`, 8 tests in `tests/test_capture_device.py`, commit `3b87b14`, 2026-07-24; `crt-console.sh`'s explicit `CRT_AUDIO_DEV` pin deliberately left in place, still a hard override) — never run against potato's real `arecord -l` output (unreachable as of 2026-07-24)
- [ ] Book Game funnel re-verified end-to-end — **offline half done** (`tests/test_console_book_game_layout.sh`, a fake-tmux shim proving all 7 windows + correct boot-default compose under both `CRT_NO_IDLE_CLAUDE` layouts, commit `cba6ea4`, 2026-07-24; `test_book_game_integration.py`'s pure-Python funnel logic was already exhaustive, nothing layout-specific needed there) — still needs a real live scan re-test on potato (unreachable as of 2026-07-24) — ranked-backlog item 4
