...
- [ ] sticky-conversation-window / arm-window state machine — **code done** (`consume_arm_with_followup()`/`check_arm_timeout()` implemented in `bin/crt-wake-arm.py`, wired into `crt-stt-solo.py`, tested in `tests/test_wake_arm.py`, commit `ede80b5`, 2026-07-23) — **2026-07-28 live-verify pass: `CRT_WAKE_ARM_ENABLED=1` is already live in potato's `~/.bash_profile`** (set some prior session, not this one — default in code is still OFF, this is a per-host override). **Tried live, no clean result yet.** Multiple real attempts at a wake-then-follow-up-without-repeating-the-word: every attempt that actually omitted "potato" arrived 16s+ after its preceding wake (cross-checked against `potato-claude`'s pane — neither reached the brain, both gated out as "no wake word") — past the 12s `CRT_WAKE_ARM_SECS` window. Given tonight's confirmed potato↔mandark transcription lag (see the network-flakiness QUESTIONS.md entry), it's unclear how much of that 16s+ gap is real speaking pace vs. queueing lag eating into the window before it's even logged. **Real open question, not just an untested checkbox:** is 12s tight for natural pacing, or is the window's clock starting from the wrong reference point given lag? Needs a retry on a quiet network, or the window's timing reference reconsidered.
  ...
  **2026-08-06 (nightly-batch): the timing-reference half of that question is now fixed at the code level — the live-pacing half still needs a real retry.** `emit()` in `bin/crt-stt-solo.py` called `wake_arm.arm()`/`consume_arm_with_followup()` with an implicit `now=time.time()`, evaluated at the moment `emit()` itself runs — which is AFTER `transcribe()` returns, i.e. after however long whisper/the network took for that utterance. Two utterances rarely take equally long (queueing, network jitter), so the 12s budget was being silently taxed by an amount that varied utterance to utterance — exactly the confound the entry above flags. Fixed: the capture loop now stamps `utt_end_ts = time.time()` the instant VAD detects the utterance ended (`ended` going True), *before* `transcribe()` is called, and threads it through as `emit(text, peak, heard_at=utt_end_ts)`; `emit()` passes that straight through as `now=` to both `wake_arm.arm()` and `consume_arm_with_followup()`. The window is now measured strictly from when the person stopped talking to when they stopped talking again — transcription/network latency no longer eats into either side of it. Regression-pinned in `tests/test_wake_rearm_ceiling.py` (`test_heard_at_drives_the_deadline_not_the_wall_clock_when_emit_runs`, a decoy-clock test: the arm module's own `time.time()` is pointed at a wildly wrong value and the published deadline still comes out relative to `heard_at`, not the decoy). Full suite green (`tests/run_tests.sh`, ALL GREEN). `ssh potato` was unreachable from this box tonight (`Could not resolve hostname potato`, see report) so this is NOT live-verified — the 16s-gap live test needs to be re-run with this fix in place before concluding whether 12s itself is still too tight. If it still doesn't reach naturally-paced follow-ups, that's now a real signal about `CRT_WAKE_ARM_SECS` itself rather than a signal muddied by latency.
