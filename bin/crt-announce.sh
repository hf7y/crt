...
# enforced by a lockfile timestamp (AUDIO-DEBUG.md has the routing history).
set -euo pipefail
...
#
#   - Stamping first stops a barrage; rolling back on failure frees the
#     window once crt-tts.py's exit status proves nothing was said. Same
#     pattern as crt-idle-teaser.sh's chime() (IDLE-BAIT.md's rate-limit rule).
echo "$now" > "$LOCK"
