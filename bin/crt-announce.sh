...
# enforced by a lockfile timestamp; CRT_AUDIO_OUT_URL restores the retired
# dexter-audio-server.py bridge if potato's local ALSA path regresses.
set -euo pipefail
...
#
#     not produce two overlapping voices. Rolling back frees the window
#     the instant crt-tts.py's real exit status shows nothing was said.
echo "$now" > "$LOCK"
