#!/usr/bin/env python3
# Book Game idle-bait: pops a line into thoughts.log when the room's been quiet a while
# -- see BOOK-GAME-STYLE.md's "Idle-bait quotes" section. Mirrors bin/crt-idle-bait.sh's
# shape (poll, check quiet-time, append a line) but reuses bin/crt-book-game.py's
# registry/quote/entice logic instead of a hardcoded LINES array. ENTICE_RATE/BIBQUOTES_RATE
# mix three registers so an empty registry never goes silent. NON-API-BY-DESIGN (see
# crt-book-game.py's pick_bibquotes_line); see tests/test_book_idle_bait.py.
...
def main():
    ...
    # pick_idle_quote(), and the getmtime() below races stt.log's exists-then-stat -- either used to end this loop outright. Guard now wraps the whole body.
