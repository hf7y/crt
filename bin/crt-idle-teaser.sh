...
chime() {
  ...
  #
  # Stamp-before-play, roll-back-only-on-failure ordering: see
  # tests/test_idle_teaser.sh's "a chime that never played left the shared
  # window unspent" / "a failed chime restores the previous stamp, not
  # zero" cases for why.
