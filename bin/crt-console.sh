...
# Real bug found 2026-07-24: nothing exported this in the console's boot
# path, so the CTL-file live-tune mechanism (crt-ring.sh, the "mute" flag
# crt-tts.py/crt-earcon.sh write during playback, crt-midi-knobs.py) was
# silently dead on potato. Witnessed: tests/test_console_ctl_env_export.sh.
...
# Flash the current IP on the real tty for CRT_IP_FLASH_SECS before the
# console takes over -- mDNS (install.sh's avahi-daemon) doesn't cross a
# router/VLAN, so this is the fallback that always works, skipped only on
# a reattach (has-session above already exited).
...
# Wrap long commands with `; exec bash`: an exit (claude quits, stt-solo
# crashes) drops to a shell, not a collapsed session. acceptEdits is
# default since interactive prompts are painful hands-free (override via
# CRT_CLAUDE_ARGS). CRT_NO_IDLE_CLAUDE=1 swaps a resident idle Claude for
# the screensaver, escalating to mandark's remote Claude -- see POTATO.md.
...
# Window 1 -- visible "monologue" pane: claude's own DIALOGUE replies,
# ephemeral (crt-monologue.py fades old lines). The one background window
# meant to be looked at (prefix+1). Wired into boot code, not just
# documented, after this exact layout vanished with no trace once
# already -- see AUDIO-DEBUG.md "Approach B".
...
# window 0) -- see AUDIO-DEBUG.md Approach B for why single-reader avoids
# a whole class of staleness bugs. SINK=secretary + GATE=1 route non-wake
# utterances through crt-secretary.py's playbooks -- see crt-stt-solo.py's
# own SINK/GATE comments. Console-wide config is NOT retyped here: the
# supervisor sources crt-conf.sh itself (2026-07-29 fix, see its header).
...
# (2026-07-21 pivot). The old dexter->8993 HTTP receiver was retired
# 2026-07-23 (Zach: "kill scanner feed, keep claude") -- it collided with
# the remote-Claude bridge's tunnel on the same port.
...
# Background daemon, not a display surface (2026-07-28: "idlebait also
# show page92 excerpts") -- keeps ~/.crt/bibquotes.txt fresh from
# bibliothecaire's published-quotes share so crt-book-idle-bait.py's
# render path never makes a network call at idle time; excerpts surface
# through the existing bookidle window, no new screen needed.
...
# -> question -> spoken answer -> STT training log). Watches
# ~/.crt/stt.log (crt-stt-solo.py writes every recognized utterance there)
# for the next utterance after a scan and grades it automatically -- see
# crt-book-answer-listen.py's own header.
...
# Background: periodically recomputes mishear candidates from the Book
# Game training log (crt-book-game-stats.py's generate_candidate_fixups())
# and auto-merges new ones into stt-fixups.json, tagged confidence:"auto",
# never touching a human-reviewed entry -- see crt-stt-training-merge.py's
# own header.
...
# -- a scan's raw keystrokes land on whichever window has focus, regardless
# of which "should" have them (SCANNER.md). In idle-lean layout the
# screensaver is the idle face instead, so it gets focus there. Witnessed:
# tests/test_console_book_game_layout.sh.
if [ "${CRT_NO_IDLE_CLAUDE:-0}" = "1" ]; then
