...
# --- Playbook: calibrate --------------------------------------------------
# DISPLAY-CALIBRATION.md's single-shot entry point. Only `show` -- `run`'s
# own input() loop doesn't fit handle()'s one-shot request/response shape.
...
# SSH server, and potato reaching in was flagged as a real vulnerability --
# see bin/crt-remote-claude-bridge.py's header for the full threat model.
...
def _confirm_in_background(text, local_answer):
    ...
    if not send_to_claude(text):
        # No call to record -- see test_confirm_in_background_no_state_change_when_send_fails.
        return
    reply, status = wait_for_claude_reply(before)
    if status != "ok":
        # Same reasoning -- see test_confirm_in_background_no_state_change_when_reply_undelivered.
        return
    with _CONFIDENCE_STATE_LOCK:
        state = stt_confidence.load_state()
