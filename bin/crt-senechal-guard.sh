...
# A heredoc BODY is literal text, not a command -- stripped (keeping the
# redirect line itself) before scanning, so a body that merely mentions
# ~/.local/share/bin in prose can't combine with an unrelated write
# elsewhere into a false positive. Witnessed by
# tests/test_senechal_guard.sh's heredoc-body cases.
...
# A "->" arrow, a stderr-to-void redirect (`2>/dev/null`, `&>/dev/null`),
# an fd-duplicating redirect (`2>&1`, `1>&2`), or an embedded ">="
# comparison (e.g. awk's `count>=3`) all contain the bare ">" WRITE_VERB
# checks for but write nothing anywhere -- stripped before checking.
# Witnessed by tests/test_senechal_guard.sh's arrow/stderr-null/fd-dup/>=
# cases.
cmd_for_write_check="$(printf '%s' "$cmd_scan" | sed -E 's/->//g; s/[0-9&]*>>?[[:space:]]*\/dev\/null//g; s/[0-9]*>&[0-9]+//g; s/>=//g')"
# A write must TARGET ~/.local/bin or ~/.local/share, not merely co-occur
# with one elsewhere in the command (e.g. a `cd` through the checkout
# followed by an unrelated write) -- anchor a write-verb command to its
# own argument list and a redirect to its own immediate target. Witnessed
# by tests/test_senechal_guard.sh's cd-then-write-elsewhere case.
