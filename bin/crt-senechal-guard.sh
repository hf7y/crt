...
# A heredoc BODY is text landing wherever its redirect points, not a
# command -- strip it (keeping the redirect line itself, so a real write
# via heredoc still matches) or path-mentions in heredoc prose false-positive.
...
# "->" (echoed text), a stderr-to-void redirect, an fd-duplicating redirect,
# and ">=" inside embedded script code (e.g. awk `count>=3`) all contain a
# bare ">" but write nothing -- strip them before checking for a write verb,
# or a read-only command merely referencing ~/.local/bin or ~/.local/share
# false-positives on it. Witnessed by tests/test_senechal_guard.sh's
# redirect/arrow/>= cases.
...
# with one elsewhere in the command -- anchor a write-verb to its own
# argument list (not past a `;`/`&`/`|` boundary) and a `>`/`>>` redirect to
# its own immediate target. Witnessed by tests/test_senechal_guard.sh's
# cd-then-unrelated-write case.
