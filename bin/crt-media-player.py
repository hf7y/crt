...
#
# KNOWN CONFLICT, found while wiring this up, NOT resolved here --
# flagging for a human, not guessing: crt-stt-solo.py's own CONTROL dict
# (voice-control keystrokes) already claims "next" -> Down arrow, sent
# straight into the tmux pane in CRT_STT_SINK=secretary mode BEFORE
# crt-secretary.py's playbooks (including this one) ever see the
# utterance -- see that file's `is_control` check. So saying "next" to
# skip a track would never reach this playbook at all; it'd navigate
# whatever's on screen in the Claude pane instead. This is exactly the
# kind of cross-persona ambiguity PERSONA-CHANNEL.md's channel-switch
# idea is meant to resolve (does "next" mean "navigate the terminal" or
# "skip the song" depends on which mode/persona is active) -- not
# something to silently patch around here by renaming a trigger word.
#
