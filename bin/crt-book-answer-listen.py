...
# against that scan's pending question, reusing crt-book-game.py's own
# grade_answer()/log_training_row() -- no new Claude/API call or STT engine.
# See tests/test_book_answer_listen.py for the pending-question window
# logic and env vars.
import calendar
...
def main():
    conn = bg.get_db()
    # LAST link of the funnel: before 2026-07-25 one raising utterance
    # (bad sqlite row, malformed JSON) ended this loop for the rest of the
    # console's uptime. See crt_loop_guard.py's LoopGuard docstring.
