#!/usr/bin/env python3
# Closes the last open link in the Book Game funnel (idle-bait -> scan -> question ->
# SPOKEN ANSWER -> STT training log, .claude/FOCUS.md's 2026-07-21 end-goal): watches
# ~/.crt/stt.log (crt-stt-solo.py's own log) for the next utterance after a scan and
# grades it. Own file (crt-book-console.py/crt-book-game.py were mid-live-debug that
# day); reuses grade_answer()/log_training_row(). See tests/test_book_answer_listen.py
# for window logic; env vars: CRT_BOOKS_DB, CRT_STT_LOG, CRT_THOUGHT_LOG, CRT_BOOK_ANSWER_WINDOW_SECS.
import calendar
...
def main():
    ...
    # row), and log_training_row's write -- any of which killed the loop silently. See crt_loop_guard.py's LoopGuard docstring.
