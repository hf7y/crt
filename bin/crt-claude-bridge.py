#!/usr/bin/env python3
# Mirrors the in-VM claude's actual replies into ~/.crt/thoughts.log, so window 1's
# ephemeral pretty-print pane (crt-monologue.py) shows claude's own responses instead of
# a separate external narration. Tails claude's own session transcript (JSONL, same
# format Claude Code always writes) rather than screen-scraping the tmux pane --
# structured, far less fragile. Bugs found here are regression-tested in tests/test_claude_bridge.py; marker convention documented in CLAUDE.md.
