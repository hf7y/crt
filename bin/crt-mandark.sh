...
# few places (POTATO.md's "wake routing" section). Preferred: mandark,
# so potato holds no Claude process at all (saves ~37% of its 1GB RAM,
...
# see tests/test_mandark_toggle.sh for why that argument exists.
probe_bridge() {
  python3 - "$1" <<'PY'
import socket, sys
port = int(sys.argv[1])
